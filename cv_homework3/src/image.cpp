...
void Image2D::InitImg()
{
  ...
  // 初始化所有二维向量
  img_.resize(height, std::vector<int>(width, 0));
  outline_.resize(height, std::vector<int>(width, 0));
  f_.resize(height, std::vector<double>(width, 0));
  phi_.resize(height, std::vector<double>(width, 0));
  Gx_.resize(height, std::vector<int>(width, 0));
  Gy_.resize(height, std::vector<int>(width, 0));
  ...
  for (int i = 0; i < height; i++)
  {
    ...
    {
      std::cin >> img_[i][j];
      if (img_[i][j] == 255) {  // 收集白色点（二值化后的前景）
        pts_.push_back(Point(i, j));
      }
    }
  }
}
...
void Image2D::SaveBox(const std::string& filename) {
  std::ofstream out(filename);
  auto box = Geometry::BoundingBox(pts_);
  out << box.size() << '\n';
...
void Image2D::SaveHull(const std::string& filename) {
  auto hull = Geometry::ConvexHull(pts_);
  std::ofstream out(filename);
  out << hull.size() << '\n';
...
void Image2D::SaveMinBox(const std::string& filename) {
  auto hull = Geometry::ConvexHull(pts_);
  auto box = Geometry::MinBox(hull);
  std::ofstream out(filename);
  out << box.pts.size() << '\n';
  for (const auto& p : box.pts) {
      out << p.y << " " << p.x << '\n';  // 坐标转换
  }
  out << box.area << '\n';
}